...
from models import OrderCategory
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"total_amount": -100}, False, 0.0, 0.0, ["больше 0"]),
        ...
        ({"user_created_at": datetime.now() - timedelta(days=7, seconds=1), "total_amount": 20000}, False, 0.0, 0.0, ["нового пользователя"]),
        ({"user_created_at": datetime.now() - timedelta(days=7), "total_amount": 20000}, False, 0.0, 0.0, ["нового пользователя"]),
        
        # Количество позиций
        ({"items_count": 1}, True, 0.0, 0.0, []),
        ({"items_count": 50}, True, 0.0, 0.0, []),
        ({"items_count": 51}, False, 0.0, 0.0, ["превышает 50"]),
        ({"items_count": 0}, False, 0.0, 0.0, ["превышает 50"]),  # Pydantic не пропустит, но проверим
        ...
        ({"category": OrderCategory.ALCOHOL.value, "age_verified": True, "created_at": datetime.now().replace(hour=0, minute=0)}, False, 0.0, 0.0, ["08:00 до 23:00"]),
        
        # Риск-скоринг
        ({"total_amount": 100001}, True, 0.9, 0.9, []),
        ({"total_amount": 100000}, True, 0.0, 0.0, []),
        ({"total_amount": 99999.99}, True, 0.0, 0.0, []),
        ({"user_email_changed_at": datetime.now() - timedelta(minutes=30)}, True, 0.2, 0.2, []),
        ({"user_email_changed_at": datetime.now() - timedelta(minutes=59, seconds=59)}, True, 0.2, 0.2, []),
        ({"user_email_changed_at": datetime.now() - timedelta(hours=1)}, True, 0.0, 0.0, []),
        ({"user_email_changed_at": datetime.now() - timedelta(hours=1, seconds=1)}, True, 0.0, 0.0, []),
        ...
        ({"total_amount": 50000, "user_email_changed_at": datetime.now() - timedelta(minutes=10), 
          "delivery_country": "RU", "wallet_country": "US"}, True, 0.5, 0.5, []),  # 0.0+0.2+0.3=0.5
        ({"total_amount": 200000, "user_email_changed_at": datetime.now() - timedelta(hours=2), 
          "delivery_country": "RU", "wallet_country": "US"}, True, 1.0, 1.0, []),  # 0.9+0+0.3=1.2 -> cap 1.0
        ...
        ({"user_created_at": datetime.now() - timedelta(days=10), "total_amount": 20000, 
          "category": OrderCategory.ALCOHOL.value, "age_verified": False, 
          "created_at": datetime.now().replace(hour=12, minute=0)}, False, 0.0, 0.0, 
          ["нового пользователя", "подтверждение возраста"]),
    ]
)
...
@given(order_strategy())
def test_valid_always_boolean(order):
    validator = FakeValidator(chaos_mode=False)
    result = validator.validate_order(order)
    assert isinstance(result["valid"], bool)

@given(order_strategy())
def test_risk_score_monotonicity(order):
    validator = FakeValidator(chaos_mode=False)
    
    # Берем заказ и увеличиваем сумму
    order1 = order.copy()
    order1["total_amount"] = max(order["total_amount"] / 2, 0.01)
    order2 = order.copy()
    order2["total_amount"] = min(order["total_amount"] * 2, 999999.99)
    
    result1 = validator.validate_order(order1)
    result2 = validator.validate_order(order2)
    
    # При увеличении суммы риск не должен уменьшаться
    if order2["total_amount"] >= order1["total_amount"]:
        assert result2["risk_score"] >= result1["risk_score"] - 0.001  # погрешность float
...
def test_alcohol_time_boundary():
    ...
    with freeze_time("2026-06-16 22:59:59"):
        order = create_order(
            category=OrderCategory.ALCOHOL.value,
            age_verified=True,
            created_at=datetime.now()
    ...
    with freeze_time("2026-06-16 23:00:01"):
        order = create_order(
            category=OrderCategory.ALCOHOL.value,
            age_verified=True,
            created_at=datetime.now()
...
def test_chaos_mode():
    validator = FakeValidator(chaos_mode=True, chaos_probability=1.0)  # 100% вероятность хаоса
    order = create_order()
    result = validator.validate_order(order)
    
    # В режиме хаоса результат может быть любым, но структура должна сохраняться
